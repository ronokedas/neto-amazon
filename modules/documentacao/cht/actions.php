...
if ($action === 'salvar') {
    ...
    try {
        if ($editando) {
            $sql = "UPDATE certificados_cht SET profissional_empresa=:profissional_empresa, cpf_cnpj=:cpf_cnpj, atividade_homologada=:atividade_homologada, observacoes=:observacoes, data_emissao=:data_emissao, assinante_nome=:assinante_nome, assinante_titulo=:assinante_titulo, assinante_registro=:assinante_registro, status=:status WHERE id=:id";
            ...
            $num = gerarNumeroDocumento('REL-HT', 'AM-REL-HT');
            $token = bin2hex(random_bytes(32));
            $id = gerarUUID();
            $sql = "INSERT INTO certificados_cht (id, numero_relatorio_ht, token_assinatura, profissional_empresa, cpf_cnpj, atividade_homologada, observacoes, data_emissao, assinante_nome, assinante_titulo, assinante_registro, status, criado_por) VALUES (:id, :numero_relatorio_ht, :token_assinatura, :profissional_empresa, :cpf_cnpj, :atividade_homologada, :observacoes, :data_emissao, :assinante_nome, :assinante_titulo, :assinante_registro, :status, :criado_por)";
            $stmt = $pdo->prepare($sql);
            $stmt->execute([
                ':id' => $id,
                ':numero_relatorio_ht' => $num,
