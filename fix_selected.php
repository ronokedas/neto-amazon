...
foreach ($files as $f) {
    ...
    // <?php if (!empty($_GET['agendamento_id']) && isset($dadosPre['embarcacao_id']) && $dadosPre['embarcacao_id'] == $emb['id']): \r\n>selected<?php endif; \?>
    
    // First, let's normalize it to something clean
    $c = preg_replace(
    ...
    // Some files might still have the old '?>selected<?php endif; ?>?>' if my previous string replace failed
    $c = preg_replace(
        '/<\?php if \(!empty\(\$_GET\[\'agendamento_id\'\]\) && isset\(\$dadosPre\[\'embarcacao_id\'\]\) && \$dadosPre\[\'embarcacao_id\'\] == \$emb\[\'id\'\]\):\s*\?>selected<\?php endif; \?>\?>/s',
        "<?php echo (!empty(\$_GET['agendamento_id']) && isset(\$dadosPre['embarcacao_id']) && \$dadosPre['embarcacao_id'] == \$emb['id']) ? 'selected' : ''; ?>",
        $c
